...
export const Flowchart = ({ q }) => {
                            ...
                            >
                                ...
                                >
                                    {rectange
                                        ...
                                        .map((part, partIndex) => (
                                            <React.Fragment
                                                key={`${stepIndex}_${rectangeIndex}_${partIndex}`}
                                            >
                                                <Text
                                                    style={{
                                                        display: "inline",
    ...
    function drawArrows() {
        ...
        var _arrows = [];
        for (var i = 0; i < q.steps.length - 1; i++) {
            // i is the step index
            for (var j = 0; j < q.steps[i].length; j++) {
                // j is the rectangle index
                for (var k = 0; k < q.steps[i + 1].length; k++) {
                    // k is the next rectangle index
                    var item1 = document.querySelector(
                        `[data-step-index="${i},${j}"]`
                    );
                    var item2 = document.querySelector(
                        `[data-step-index="${i + 1},${k}"]`
                    ); // next step number k
    ...
    return (
        <>
            {result}
            <svg className="arrow-container">{arrows}</svg>
        </>
    );
};
